...
import baseUser from "@/mixins/user/baseUser";

export default {
  name: "Edit",
  components: { ToastBase, SubmitButton, StringInput, CenterLayout },
  mixins: [validation, baseUser],
  data() {
    return {
      phone_number: "",
      form: {
  ...
  async mounted() {
    let response = await users.getUser(this.playerId);
  ...
  methods: {
    async submit() {
      ...
      if (!this.$v.$invalid) {
        try {
          await this.updateUser(this.playerId, this.form);
